    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            half4 frag(v2f IN) : SV_Target
            {
                ...
                // Decode GLB raw-RGB normal map (GLTFast embeds normals as plain RGB, not Unity-packed DXT5nm)
                if (_UseNormalMap > 0.5)
                {
                    float3 normalTS = SAMPLE_TEXTURE2D(_BumpMap, sampler_BumpMap, IN.uv).rgb * 2.0 - 1.0;
